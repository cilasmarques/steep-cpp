                                     ...
                                     double *ustarWLine, double *rahRLine, double *rahWLine, double *a, double *b, double *u200, int *size)
{
  ...
  while (pos < *size)
  {
    ...
    double ust = (VON_KARMAN * ustarRLine[pos]) / (log((10 - DISP) / zomLine[pos]) - psi200);

    double zoh_terra = zomLine[pos] / pow(exp(1.0), (kb1_vector[pos]));
    double temp_rah1_corr_terra = (ust * VON_KARMAN);
    double temp_rah2_corr_terra = log((10 - DISP) / zomLine[pos]) - psi2;
    double temp_rah3_corr_terra = temp_rah1_corr_terra * log(zomLine[pos] / zoh_terra);
